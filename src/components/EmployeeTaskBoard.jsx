...
const EmployeeTaskBoard = ({ projectId }) => {
  ...
  const handleUpdate = (task) => {
    ...
    dispatch(updateTaskStatus({ id: task.id, status: updatedStatus, comments: updatedComment }));
  ...
  return (
    <div className="space-y-6">
      {employees.length === 0 ? (
        <p className="text-gray-500 italic">No employees assigned to this project.</p>
      ) : (
        employees.map((emp) => {
          const empTasks = tasks.filter(
            (task) => task.assignedTo === emp.email && task.projectId === projectId
          );
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-sm">📅 Due: {task.dueDate || "N/A"}</p>
                        </div>
